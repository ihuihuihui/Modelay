...
pub fn build_prompt(record: &ThreadRecord, content: &TodayContent) -> String {
    ...
    for (role, text) in content.messages.iter().rev() {
        if used >= MAX_EXCERPT_CHARS { break; }
    ...
    let paths = if content.referenced_paths.is_empty() {
        ...
        content.referenced_paths.iter().map(|path| format!("- {path}")).collect::<Vec<_>>().join("\n")
    };
    format!(r#"这是 Modelay 从旧任务自动生成的续接任务。不要加载或复述旧任务的完整历史，只依据当前工作区和下面的精简交接继续。
...
6. 完成整理后继续执行任务，并把关键产物路径清楚汇报给用户。"#,
        record.id, record.title, record.cwd, record.tokens_used,
        paths,
...
fn read_thread(thread_id: &str) -> Result<ThreadRecord> {
    ...
    connection.query_row(
        &query,
        [thread_id],
        |row| Ok(ThreadRecord { id: row.get(0)?, title: row.get(1)?, cwd: row.get(2)?, provider: row.get(3)?, model: row.get(4)?, effort: row.get(5)?, tokens_used: row.get(6)?, updated_at_ms: row.get(7)?, rollout_path: row.get(8)? }),
    ).map_err(|error| match error {
        rusqlite::Error::QueryReturnedNoRows => ModelayError::Message("没有找到该用户任务，请检查会话 ID。".into()),
        other => other.into(),
    })
}
...
fn read_today_content(record: &ThreadRecord) -> Result<TodayContent> {
    ...
    for path in files {
        ...
        for line in reader.lines().map_while(std::result::Result::ok) {
            ...
            let Some(timestamp) = value.get("timestamp").and_then(Value::as_str) else { continue };
            let Ok(parsed) = DateTime::parse_from_rfc3339(timestamp) else { continue };
            if parsed.with_timezone(&Local).date_naive() != today { continue; }
            let Some((role, text)) = message_text(&value) else { continue };
...
fn rollout_files(record: &ThreadRecord) -> Result<Vec<PathBuf>> {
    ...
    for root in [paths::codex_dir()?.join("sessions"), paths::codex_dir()?.join("archived_sessions")] {
        visit(&root, &record.id, &mut result)?;
    }
    let direct = PathBuf::from(&record.rollout_path);
    if direct.is_file() && !result.contains(&direct) { result.push(direct); }
    if result.is_empty() { return Err("没有找到该任务的本地会话记录。".into()); }
    Ok(result)
}
...
fn visit(directory: &Path, needle: &str, result: &mut Vec<PathBuf>) -> Result<()> {
    ...
    for item in fs::read_dir(directory)? {
        let path = item?.path();
        if path.is_dir() { visit(&path, needle, result)?; }
        else if path.extension().and_then(|value| value.to_str()) == Some("jsonl")
            && path.file_name().and_then(|value| value.to_str()).is_some_and(|name| name.contains(needle)) { result.push(path); }
    }
    Ok(())
}
...
fn message_text(value: &Value) -> Option<(String, String)> {
    ...
    if payload.get("type")?.as_str()? != "message" { return None; }
    let role = payload.get("role")?.as_str()?;
    if !matches!(role, "user" | "assistant") { return None; }
    let text = payload.get("content")?.as_array()?.iter().filter_map(|item| {
...
fn collect_paths(text: &str, result: &mut BTreeSet<String>) {
    for token in text.split_whitespace() {
        ...
        if (cleaned.starts_with('/') || (cleaned.len() > 3 && cleaned.as_bytes().get(1) == Some(&b':'))) && cleaned.len() <= 500 {
...
fn validate_thread_id(value: &str) -> Result<()> {
    ...
    if value.len() < 16 || value.len() > 80 || !value.bytes().all(|byte| byte.is_ascii_alphanumeric() || byte == b'-') {
...
fn compact(text: &str, limit: usize) -> String {
    ...
    if normalized.chars().count() <= limit { normalized } else { format!("{}…", normalized.chars().take(limit).collect::<String>()) }
}

fn numbered(items: &[String]) -> String {
    if items.is_empty() { return "1. 当天没有可提取的对应消息，请以工作区文件为准。".into(); }
    items.iter().enumerate().map(|(index, item)| format!("{}. {}", index + 1, item)).collect::<Vec<_>>().join("\n")
}
...
mod tests {
    ...
    fn handoff_prompt_is_bounded_and_structured() {
        let record = ThreadRecord { id: "thread-id".into(), title: "Project".into(), cwd: "/tmp/project".into(), provider: "custom".into(), model: "gpt".into(), effort: "medium".into(), tokens_used: 2_000_000, updated_at_ms: 0, rollout_path: String::new() };
        let content = TodayContent { messages: vec![("user".into(), "最新需求".repeat(30_000)), ("assistant".into(), "已完成 A".into())], bytes: 1, referenced_paths: vec!["/tmp/project/a.md".into()] };

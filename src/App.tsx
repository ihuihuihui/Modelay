...
const sessionScopeLabels: Record<SessionScope, string> = { recent5: "最近活动的 5 个任务", all: "覆盖全部旧任务", single: "指定一个会话 ID" };
...
function App() {
  ...
  async function inspectThread() {
    ...
    setHandoffBusy(true); setThreadHealth(null); setHandoffReport(null); setHandoffError(null); setError(null); setMessage("正在读取当天会话记录并分析风险…");
    try {
  ...
  async function createHandoff() {
    ...
    setHandoffBusy(true); setHandoffError(null); setError(null); setMessage("正在整理当天需求、项目资料和进度，并创建新任务…");
    try {
    ...
    } catch (reason) { const detail = String(reason); setHandoffError(detail); setError(detail); setMessage("续接任务创建失败，旧任务未受影响"); }
    finally { setHandoffBusy(false); }
  }
  ...
  return <div className="shell">
    ...
    <main className="content">
      ...
      <section className="panel handoff-panel">
        <div className="handoff-workflow"><div className="panel-head"><div><h2>会话续接助手 <span className="beta-badge">内测</span></h2><p>检查上下文风险，并将当天项目资料、最新需求与进度交接到全新任务</p></div><Activity size={20} className={threadHealth?.riskLevel === "critical" ? "risk-critical" : threadHealth?.riskLevel === "warning" ? "orange" : "green"} /></div>
          <div className="handoff-beta-note" role="note"><AlertTriangle size={16} /><span><strong>内测功能 · 还在测试中尚不稳定</strong>续接会读取当天会话摘要并创建新任务；如果失败，旧任务不会被修改。建议先保存工作区，再进行测试。</span></div>
          <div className="handoff-input"><label htmlFor="handoff-thread-id">会话 ID</label><div><input id="handoff-thread-id" value={handoffThreadId} onChange={(event) => { setHandoffThreadId(event.target.value); setThreadHealth(null); setHandoffReport(null); setHandoffError(null); }} placeholder="例如 01a041f4-ef73-7560-b418-c930ab8b6af0" spellCheck={false} /><button className="ghost-btn" onClick={() => void inspectThread()} disabled={handoffBusy}>{handoffBusy && !threadHealth ? <RefreshCw size={14} className="spin" /> : <Activity size={14} />}检查会话</button></div></div>
          {handoffError && <div className="handoff-error" role="alert"><AlertTriangle size={17} /><div><strong>续接任务没有创建完成</strong><span>{handoffError}</span><small>请确认当前渠道已经启用并重启 Codex，然后重新检查该会话再试一次。旧任务不会被修改。</small></div></div>}

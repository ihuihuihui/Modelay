...
- 官方/第三方动态模型、推理强度、真实额度、任务索引备份与用户任务 Provider/模型/推理强度覆盖。跨渠道切换可选择最近 5 个、全部或指定会话，让耗尽额度的原会话继续使用目标渠道；切换前自动备份任务索引。多额度桶会明确优先 Codex 桶，短周期标签按服务端 `windowDurationMins` 显示，不会把 15 分钟或 1 小时误写成 5 小时。

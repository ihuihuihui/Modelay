...
- 官方/第三方动态模型、推理强度、真实额度、任务索引备份与用户任务 Provider/模型/推理强度覆盖。跨渠道切换固定同步全部可互通用户任务，避免旧 Provider 在新配置下无法加载；重新应用当前渠道时仍可选择最近 5 个、全部或指定会话。多额度桶会明确优先 Codex 桶，短周期标签按服务端 `windowDurationMins` 显示，不会把 15 分钟或 1 小时误写成 5 小时。
...
- 会话续接助手可按会话 ID 检测累计 tokens、当天记录体积和消息数量，提示上下文过大风险，并把当天需求、进度与项目路径写入精简的新任务；交接消息被 Codex 接受后立即停止空转并释放会话，也不会修改或复制旧任务完整历史。
...
- Rust 单元测试：37 项通过。
- TypeScript 悬浮窗、额度标签、模型选择、跨渠道范围、版本说明和更新状态测试：18 项通过。

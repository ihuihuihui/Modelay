...
- 会话续接助手可按会话 ID 检测累计 tokens、当天记录体积和消息数量，提示上下文过大风险，并把当天需求、进度与项目路径直接注入精简的新任务；不会后台启动模型轮次、长期占用新会话，也不会修改或复制旧任务完整历史。
...
- Rust 单元测试：36 项通过。
- TypeScript 悬浮窗、额度标签、模型选择、跨渠道范围和更新状态测试：16 项通过。

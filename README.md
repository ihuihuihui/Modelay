...
- 官方/第三方动态模型、推理强度、真实额度、任务索引备份与用户任务 Provider/模型/推理强度覆盖。跨渠道切换默认保留原任务，并在目标渠道创建精简续接任务，避免第三方渠道重复处理完整长上下文；仍可选择仅切换渠道，或按最近 5 个、全部、指定会话直接迁移。多额度桶会明确优先 Codex 桶，短周期标签按服务端 `windowDurationMins` 显示，不会把 15 分钟或 1 小时误写成 5 小时。
...
- 会话续接助手检测最近一次真实输入 tokens、累计 tokens、当天记录体积和消息数量，并把当天需求、进度与项目路径写入精简的新任务；最近输入达到 4 万 tokens 时提示延迟风险，达到 8 万时明确建议智能续接。交接消息被 Codex 接受后立即停止空转并释放会话，也不会修改或复制旧任务完整历史。
